...
import { joinChallenge, getParkLeaderboard } from "@/src/lib/api";
import type { Challenge, FriendChallengeProgress, LeaderboardEntry } from "@/src/lib/types";
import { trackEvent } from "@/src/lib/analytics";
import { FollowButton } from "@/src/components/follow-button";
import type { BenchWithStatus, ChallengePageData } from "./load-challenge-data";
...
type UpcomingChallenge = { id: string; title: string; description: string };
...
export function ChallengeContent({ data }: { data: ChallengePageData }) {
  const searchParams = useSearchParams();
  const { profileId, allChallenges, greenLakeChallenge: challenge, benchList, friendProgress } = data;
  const selectedChallengeId = searchParams.get("challenge");

  const [leaderboard, setLeaderboard] = useState<LeaderboardEntry[]>(data.leaderboard);
  ...
  if (showLanding) {
    return (
      <LandingView
        ...
        inactiveUpcomingChallenges={inactiveUpcomingChallenges}
        completedChallenges={completedChallenges}
        pastChallenges={pastChallenges}
        completedBenches={completedBenches}
  ...
  return (
    <section className="screen">
      ...
      <FriendsSection friendProgress={friendProgress} startedFriends={startedFriends} completedFriends={completedFriends} />
      <TrailSection benchList={benchList} />
      <AchievementsSection completedBenches={completedBenches} />
      ...
      <BenchChecklist benchList={benchList} />
      <LeaderboardSection leaderboard={leaderboard} profileId={profileId} totalBenches={totalBenches} />
...
function LandingView({
  ...
  inactiveUpcomingChallenges: Challenge[];
  ...
  return (
    <section className="screen">
      ...
      <ChallengeSection title="active" challenges={activeChallenges} profileId={profileId} accentBorder completedBenches={completedBenches} totalBenches={totalBenches} emptyText="no active challenges" />
      <UpcomingSection challenges={inactiveUpcomingChallenges} profileId={profileId} />
      <ChallengeSection title="completed" challenges={completedChallenges} profileId={profileId} accentBorder completed emptyText="no completed challenges yet" />
      <ChallengeSection title="past" challenges={pastChallenges} profileId={profileId} dimmed emptyText="no past challenges" />
    </section>
  );
}
...
function ChallengeSection({
  ...
  const borderColor = accentBorder ? "var(--accent)" : "var(--border)";
  return (
    <section>
      <h2 style={{ fontSize: 14, margin: "0 0 8px", textTransform: "lowercase" }}>{title}</h2>
      <div style={{ display: "grid", gap: 8 }}>
        {challenges.length === 0 ? <p className="muted" style={{ margin: 0 }}>{emptyText}</p> : null}
        {challenges.map((c) =>
          profileId ? (
            <Link key={c.id} href={`/challenges?challenge=${encodeURIComponent(c.id)}`} style={{ textDecoration: "none" }}>
              <div className="surface-card" style={{
                ...
                background: completed ? "var(--accent-soft)" : undefined
              }}>
                <p style={{ margin: 0, fontWeight: 700, fontSize: 15 }}>{c.title}{completed ? " ✅" : ""}</p>
                <p className="muted" style={{ margin: "4px 0 0", fontSize: 12 }}>{c.description}</p>
                {c.id === GREEN_LAKE_CHALLENGE_ID && completedBenches !== undefined && (
                  <p style={{ margin: "8px 0 0", fontSize: 12, color: "var(--accent)", fontWeight: 600 }}>
                    your progress: {completedBenches}/{Math.max(totalBenches ?? 8, 8)}
                  </p>
                )}
                {dimmed && (
            ...
            <div key={c.id} className="surface-card" style={{
              padding: 14, borderLeft: `3px solid ${borderColor}`, opacity: 0.75,
              background: completed ? "var(--accent-soft)" : undefined
            }}>
              <p style={{ margin: 0, fontWeight: 700, fontSize: 15 }}>{c.title}{completed ? " ✅" : ""}</p>
              <p className="muted" style={{ margin: "4px 0 0", fontSize: 12 }}>{c.description}</p>
...
function UpcomingSection({ challenges, profileId }: { challenges: Challenge[]; profileId: string | null }) {
  return (
    <section>
...
function FriendsSection({ friendProgress, startedFriends, completedFriends }: {
  friendProgress: FriendChallengeProgress[];
  startedFriends: FriendChallengeProgress[];
  completedFriends: FriendChallengeProgress[];
}) {
  return (
            ...
            {friendProgress.map((friend) => (
              <Link
                ...
                style={{
                  ...
                  border: "1px solid var(--border)", borderRadius: 10, padding: "8px 10px", textDecoration: "none"
                }}
              >
...
function TrailSection({ benchList }: { benchList: BenchWithStatus[] }) {
  return (
        ...
        {benchList.map((b, i) => {
          ...
          return (
            <div key={b.id} style={{ display: "flex", alignItems: "center" }}>
              {i > 0 && <div className={`trail-connector${prevDone && b.benchmarked ? " completed" : ""}`} />}
...
function AchievementsSection({ completedBenches }: { completedBenches: number }) {
  return (
    <div>
...
function BenchChecklist({ benchList }: { benchList: BenchWithStatus[] }) {
  return (
    <div>
      <h2 style={{ fontSize: 14, fontWeight: 700, marginBottom: 8, textTransform: "lowercase" }}>
        bench checklist
...
function LeaderboardSection({ leaderboard, profileId, totalBenches }: {
